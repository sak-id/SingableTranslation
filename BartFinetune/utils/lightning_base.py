...
arg_to_scheduler = {
    ...
    "constant_with_warm_up": get_constant_schedule_with_warmup,
}
...
class BaseTransformer(pl.LightningModule):
    ...
    def get_lr_scheduler(self):
        ...
        if self.hparams.lr_scheduler != 'constant_with_warm_up':
    ...
    def on_save_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        with open(os.path.join(save_path, 'epoch_and_step.txt'), 'a') as f:
            f.write('Epoch: {} | Step: {}\n'.format(self.current_epoch, self.global_step))

        # if all_metrics['val_avg_bleu'] > self.best_bleu and self.global_step > 0:
        #     save_path = self.output_dir.joinpath("best_tfmr")
        #     self.best_bleu = all_metrics['val_avg_bleu']
        #     self.model.save_pretrained(save_path)
        #     self.tokenizer.save_pretrained(save_path)
        #     checkpoint_name = 'epoch: {}, step: {}, bleu: {}'.format(
        #         self.current_epoch, self.global_step, self.best_bleu)
        #     with open(os.path.join(save_path, checkpoint_name), 'w') as f:
        #         f.write('emmm...')
